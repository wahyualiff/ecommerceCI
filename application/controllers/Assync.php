...
class Assync extends CI_Controller
{
	...
	function bayarpesanan($wa = null)
	{
		...
		if (isset($_POST["total"]) and isset($_SESSION["usrid"])) {
			...
			$text .= "Ongkos Kirim : *Rp " . $this->func->formUang($_POST["ongkir"]) . "*\n";
			$text .= "Diskon : *Rp " . $this->func->formUang($_POST["diskon"]) . "*\n";
			$text .= "Total : *Rp " . $this->func->formUang($_POST["total"]) . "*\n";
			$text .= "------------------------------\n\n";
			$text .= "*Nama Penerima*\n";
			...
			$text .= $alamatz . "\n\n";
			$text .= "*Jasa Kurir*\n";
			$text .= strtoupper($_POST["kurir"] . " " . $_POST["paket"]);
			...
			if ($wa == null) {
				...
				$pesan = "
						...
						<a href='" . site_url("admin") . "'>Klik Disini</a>
					";
			...
			echo json_encode(array("success" => false, "message" => "forbidden", "text" => $text));
		}
	}
	...
	public function getTerimaPesanan()
	{
		if (isset($_GET["fid"])) {
			...
			if ($db->num_rows > 1) {
				echo "<option value=''>Pilih Pesanan</option>";
			}
			foreach ($db->result() as $res) {
	...
	public function cekongkir()
	{
		if ($_POST) {
			...
			if ($datakec->idkab == $daris and $kurir == "jne") {
				if ($_POST["service"] == "REG") {
					$service = "CTC";
				} elseif ($_POST["service"] == "YES") {
					$service = "CTCYES";
				}
			}
			...
			if ($arr->rajaongkir->status->code == "200") {
				...
				for ($i = 0; $i < count($arr->rajaongkir->results[0]->costs); $i++) {
					...
					if ($idhistory > 0) {
						...
						if ($harga > 0) {
							$this->db->insert("historyongkir", $array);
						}
					}
